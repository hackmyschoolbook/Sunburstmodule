  ...
  */
    updateColor() {
        this.svg.selectAll('path.sunburst-main-arc')
            .style("fill", (d) => d.parent ? this._colorize(d) : "#fff")
            .style("text", (d) => d.parent ? this._getLabelText(d) : '')
            .style("display", (d) => d.parent ? this._getLabelText(d) : '')
    }
    ...
    _create() {
        ...
        if (!this.svg) {
            ...
            gSlices.append("path")
                ...
                }).style("fill", (d) => d.parent ? this._colorize(d) : "#EFF2F5")
                .on('click', function (node) {
                    this._onClick(node)
                    this.props.onClick && this.props.onClick(node);
                    this.props.type && this._update(node)
                }.bind(this))
            ...
            if (this.props.labelFunc) {
                ...
                const text = gSlices.append('text')
                    ...
                    .style('font-size', d => d.depth === 0 ? '1.3em' : '11px')
    ...
    _setTooltips() {
        ...
        this.tooltipDom = d3Select(`#${this.domId}`)
            ...
            .style('pointer-events', 'none')
            .style('background', '#2d353d')
            .style('color', 'white')
            .style('padding', '3px')

        const dx = this.props.tooltipX - 50
        const dy = this.props.tooltipY - 370
        this.svg.selectAll('path.sunburst-main-arc')
    ...
    _colorize(d) {
        ...
        /*  const thishsl = d3Hsl(current.fill);
          ...
          const colorshift = thishsl.h + (hue * 2);
